...
5. **RAG (Knowledge Base tab):** the site owner chooses post types, taxonomies, site identity, and WooCommerce store data; ChatHearth writes markdown files and indexes them. Per-entry include/exclude. Incremental reindex of the changed entry. Vector store: WordPress database, self-hosted Chroma, or Pinecone. Chat replies cite source URLs.
...
4. The vector store could be managed locally using chroma or sqlite, you should select the most appropriate solution, this is an internally selection, the user will not have an option to select. Only one option will be used for the plugin if the local storage is selected. — **superseded in 1.4:** the site owner chooses **Chroma (self-hosted)**, **Pinecone**, or a **WordPress-database** store.
5. The vector store could be on a 3rd party solution like PineCone or SupaBase, we will specify the list of the initial providers and release them incrementally, then we can add more providers based on the customers' requests. — **Pinecone shipped in 1.4;** Supabase and others remain later.
