...
final class Chroma_Vector_Store implements Vector_Store_Interface {
	...
	private function detect_api(): string {
		...
		$v2 = wp_remote_get( $this->base_url . '/api/v2/heartbeat', array( 'timeout' => 5 ) );
		if ( ! is_wp_error( $v2 ) && 200 === (int) wp_remote_retrieve_response_code( $v2 ) ) {
			$this->api = 'v2';
			return $this->api;
		}

		$v1 = wp_remote_get( $this->base_url . '/api/v1/heartbeat', array( 'timeout' => 5 ) );
		if ( ! is_wp_error( $v1 ) && 200 === (int) wp_remote_retrieve_response_code( $v1 ) ) {
			$this->api = 'v1';
			return $this->api;
		}
